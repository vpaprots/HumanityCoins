...
docker run --rm --env-file $CCROOT/env/nvp0.env hyperledger/fabric-peer peer node start --logging-level=debug >  $CCROOT/logs/nvp0.log 2>&1 & 
sleep 2 

echo "Starting HyperLedger Fabric NON Validating Peer 2/2" 
docker run --rm --env-file $CCROOT/env/nvp1.env hyperledger/fabric-peer peer node start --logging-level=debug >  $CCROOT/logs/nvp1.log 2>&1 & 
sleep 2 
echo 
